...
extern "C" fn rust_main() -> ! {
    use virtio_blk::{BLK_SIZE, VirtIOBlk};

    print_str("[ch1] init virtio-blk...\n");

    let mut blk: VirtIOBlk = match VirtIOBlk::new() {
        Ok(drv) => drv,
        Err(err) => {
            print_str("[ch1] virtio init failed: ");
            print_blk_error(err);
            print_str("\n");
            shutdown(true);
        }
    ...
    let mut write_buf = [0u8; BLK_SIZE];
    let message = b"Hello, world!";
    write_buf[..message.len()].copy_from_slice(message);

    const TEST_SECTOR: u64 = 1;

    print_str("[ch1] write sector...\n");
    if let Err(err) = blk.write_block(TEST_SECTOR, &write_buf) {
        print_str("[ch1] write failed: ");
        print_blk_error(err);
        print_str("\n");
        shutdown(true);
    }

    let mut read_buf = [0u8; BLK_SIZE];
    print_str("[ch1] read sector...\n");
    if let Err(err) = blk.read_block(TEST_SECTOR, &mut read_buf) {
        print_str("[ch1] read failed: ");
        print_blk_error(err);
        print_str("\n");
        shutdown(true);
    }
    ...
    if read_buf == write_buf {
        print_str("[ch1] verify ok: sector data matched\n");
        shutdown(false);
    }

    print_str("[ch1] verify failed: sector data mismatch\n");
    shutdown(true)
}
...
#[cfg(target_arch = "riscv64")]
fn print_blk_error(err: virtio_blk::BlkError) {
    use virtio_blk::BlkError;

    match err {
        BlkError::Mmio(msg) => print_str(msg),
        BlkError::Queue(msg) => print_str(msg),
        BlkError::Io(0) => print_str("io status ok (unexpected path)"),
        BlkError::Io(1) => print_str("io error"),
        BlkError::Io(2) => print_str("io unsupported"),
        BlkError::Io(_) => print_str("io unknown status"),
    }
}
